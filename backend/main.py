...
async def get_library_cards_endpoint(
    ...
    This endpoint returns cards from the global library that all users can access.
    Users can browse these cards and add them to their personal wallet.
    
    Query Parameters:
    - issuer: Filter by card issuer (e.g., "Chase", "American Express", "Citi")
    - best_for: Filter by category where card performs well (e.g., "dining", "travel", "groceries")
    - annual_fee_max: Maximum annual fee to filter by
    - skip: Number of records to skip (for pagination)
    - limit: Maximum number of records to return (max 100)
    
    Example requests:
    - GET /api/v1/cards/library
    - GET /api/v1/cards/library?issuer=Chase
    - GET /api/v1/cards/library?best_for=dining
    - GET /api/v1/cards/library?annual_fee_max=50
    - GET /api/v1/cards/library?best_for=travel&annual_fee_max=100
    """
    from crud import get_library_cards
    
    try:
        cards = get_library_cards(
            db=db,
            issuer=issuer,
            best_for=best_for,
        ...
        logger.error(f"Error fetching library cards: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Error fetching library cards: {str(e)}"
        )


@app.post("/api/v1/users/{user_id}/cards/add-from-library")
async def add_card_from_library_endpoint(
    user_id: str,
    request: AddCardFromLibraryRequest,
    db: Session = Depends(get_db)
):
    """
    Add a library card to user's wallet
    
    This endpoint creates a personal copy of a library card for the user.
    The user can optionally provide the last 4 digits of their physical card.
    
    Path Parameters:
    - user_id: The ID of the user adding the card
    
    Request Body:
    - library_card_id: The ID of the library card to copy
    - last_four_digits: Optional last 4 digits of the user's card (4 digits)
    
    Example request:
    POST /api/v1/users/user_123/cards/add-from-library
    {
        "library_card_id": "lib_card_abc",
        "last_four_digits": "1234"
    }
    """
    from crud import copy_library_card_to_user, get_user
    ...
    # Copy the library card to user's wallet
    try:
        user_card = copy_library_card_to_user(
            db=db,
            user_id=user_id,
            library_card_id=request.library_card_id,
        ...
        return {
            ...
            "card": CreditCard(
                ...
                issuer=user_card.issuer,
                cash_back_rate=user_card.cash_back_rate,
                points_multiplier=user_card.points_multiplier,
                annual_fee=user_card.annual_fee,
                benefits=user_card.benefits or [],
                sign_up_bonus=user_card.sign_up_bonus,
        ...
        logger.error(f"Error adding card from library: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Error adding card from library: {str(e)}"
        )
...
async def get_user_cards_endpoint(
    ...
    """
    Get all cards for a specific user (excludes library cards)
    
    This endpoint returns only the cards that belong to the user's personal wallet.
    Library cards are NOT included in this response.
    
    Path Parameters:
    - user_id: The ID of the user
    
    Query Parameters:
    - active_only: If true, only return active cards (default: true)
    
    Example requests:
    - GET /api/v1/users/user_123/cards
    - GET /api/v1/users/user_123/cards?active_only=false
    """
    from crud import get_user, get_user_cards
    ...
    try:
        cards = get_user_cards(db, user_id, active_only=active_only)
        
        # Filter out library cards (should already be filtered by user_id, but double-check)
        # This is a safety measure to ensure library cards never appear in user's wallet
        user_cards = [card for card in cards if not card.is_library_card]
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error fetching user cards: {str(e)}"
        )


@app.delete("/api/v1/users/{user_id}/cards/{card_id}")
async def delete_card_endpoint(
    ...
    """
    Delete a credit card (cannot delete library cards)
    
    This endpoint deletes a card from the user's wallet.
    Library cards CANNOT be deleted through this endpoint.
    ...
    Security:
    - Only the card owner can delete their own cards
    - Library cards are protected from deletion
    
    Example request:
    DELETE /api/v1/users/user_123/cards/card_abc
    """
    from crud import delete_card
    
    success, error = delete_card(db, card_id, user_id)
    
    if not success:
        if "library" in error.lower():
            # Return 403 Forbidden for library card deletion attempts
            raise HTTPException(status_code=403, detail=error)
        elif "not found" in error.lower():
            raise HTTPException(status_code=404, detail=error)
        else:
            # Return 400 Bad Request for other errors (like unauthorized)
